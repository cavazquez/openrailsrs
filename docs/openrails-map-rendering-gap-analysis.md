...
1. ~~**La correspondencia grafo lógico ↔ TDB no está demostrada.**~~ **Resuelto en #26 + fix datum:** el ID `nNNNN`/alias solo se acepta si el pose TDB está a ≤25 m del grafo **absoluto** (sin `RouteWorldOffset`); el pose aceptado se recoloca con el offset para el frame de `world_anchor`. En Chiltern Birmingham, `n10778`/`n10770` quedan `mapping_method=id_validated` (`id_delta≈0`). Un ID realmente lejano sigue rechazándose (sin teletransporte).
