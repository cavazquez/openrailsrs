...
| **9** | **Vía dinámica** (perfiles TSection / mallas como `DynamicTrackPrimitive`) | 2 | 4 | Mucha lógica y datos; mejor cuando el grafo y la cámara ya estén sólidos. **✅ hecho (PR 1)** — segmentos orientados desde `Dyntrack` en `.w` (durmientes + 2 rieles, local +Z); sin perfiles TSection ni enlace `.tdb`. |
