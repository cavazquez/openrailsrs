...
use bevy::image::{
    Image, ImageAddressMode, ImageFilterMode, ImageSampler, ImageSamplerDescriptor, Volume,
};
...
struct OrVsmMomentPassLabel;

#[derive(Default)]
struct OrVsmMomentNode;
...
impl Plugin for OrVsmRenderPlugin {
    ...
    fn finish(&self, app: &mut App) {
        ...
        render_app
            ...
            .add_render_graph_node::<ViewNodeRunner<OrVsmMomentNode>>(Core3d, OrVsmMomentPassLabel)
            .add_render_graph_edges(
                Core3d,
                (
                    NodePbr::LateShadowPass,
                    OrVsmMomentPassLabel,
...
pub fn create_moment_atlas_image(size: u32, layers: u32) -> Image {
    ...
    let data = vec![0u8; pixel_size * extent.volume()];
    Image {
        data: Some(data),
        ...
        sampler: ImageSampler::Descriptor(ImageSamplerDescriptor {
            label: Some("or_moment_sampler".into()),
...
pub fn create_moment_preview_image(size: u32) -> Image {
    ...
    let data = vec![0u8; 4 * extent.volume()];
    Image {
        data: Some(data),
        data_order: bevy::render::render_resource::TextureDataOrder::default(),
        texture_descriptor: TextureDescriptor {
...
impl ViewNode for OrVsmMomentNode {
    type ViewQuery = (&'static OrVsmRenderSettings, &'static OrVsmViewBindGroups);
    ...
    fn run(
        ...
        (settings, bind_groups): QueryItem<Self::ViewQuery>,
        world: &World,
    ) -> Result<(), NodeRunError> {
        if !settings.enabled {
            return Ok(());
        ...
        let pipelines = world.resource::<OrVsmPipelines>();
        let pipeline_cache = world.resource::<PipelineCache>();
        let atlas = world.resource::<OrMomentAtlasImage>();
        let blur_temp = world.resource::<OrMomentBlurTempImage>();
        let gpu_images = world.resource::<RenderAssets<GpuImage>>();
        let render_queue = world.resource::<RenderQueue>();
        let render_device = world.resource::<RenderDevice>();

        let Some(pack_pipeline) = pipeline_cache.get_render_pipeline(pipelines.pack_pipeline)
        else {
            return Ok(());
        };
        let Some(blur_pipeline) = pipeline_cache.get_render_pipeline(pipelines.blur_pipeline)
        else {
            return Ok(());
        };
        let Some(atlas_gpu) = gpu_images.get(atlas.0.id()) else {
        ...
        for cascade in 0..MOMENT_CASCADE_COUNT {
            ...
            let blur_layer_view =
                moment_layer_view(&blur_gpu.texture, cascade, "or_moment_blur_layer");
        ...
        let debug = world.get_resource::<OrVsmDebugSettings>();
        if debug.is_some_and(|d| d.atlas_preview && d.mode == crate::or_vsm::OrVsmMode::Exact) {
            let layer = debug
                .map(|d| d.atlas_layer)
                .unwrap_or(0)
                .min(MOMENT_CASCADE_COUNT - 1);
            let preview = world.resource::<OrMomentPreviewImage>();
            if let (Some(preview_pipeline), Some(preview_gpu)) = (
                pipeline_cache.get_render_pipeline(pipelines.preview_pipeline),
                gpu_images.get(preview.0.id()),
            ) {
                let preview_layout =
                    pipeline_cache.get_bind_group_layout(&pipelines.preview_layout);
