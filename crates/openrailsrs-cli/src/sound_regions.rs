...
//! and emits `Enter` / `Leave` transitions on edge crossings.  The cab loop
//! translates those transitions into [`AudioCmd::EnterRegion`] /
//! [`AudioCmd::LeaveRegion`].
//!
//! When `scenario.sound_regions` is empty the tracker is a no-op.
...
impl RegionTracker {
    ...
    pub fn step(&mut self, edge_id: &str, position_m: f64) -> Vec<RegionTransition> {
        ...
        // Enter: in `new_active` but not in `self.active`.
        for region in &self.regions {
        ...
        // Leave: in `self.active` but no longer in `new_active`.
...
mod tests {
    ...
    fn entering_region_emits_enter_once() {
        ...
        // Stepping again inside the region must not re-emit Enter.
