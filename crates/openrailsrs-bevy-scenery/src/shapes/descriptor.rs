...
/// Open Rails: with `HasNightSubObj`, sub-object index 1 is night-only geometry.
pub fn night_subobj_part_visible(
    has_night_subobj: bool,
    sub_object_idx: u32,
    is_day: bool,
) -> bool {
    !(has_night_subobj && sub_object_idx == 1 && is_day)
}
...
mod tests {
    ...
    #[test]
    fn night_subobj_hidden_by_day() {
        assert!(!night_subobj_part_visible(true, 1, true));
        assert!(night_subobj_part_visible(true, 1, false));
        assert!(night_subobj_part_visible(true, 0, true));
        assert!(night_subobj_part_visible(false, 1, true));
    }
