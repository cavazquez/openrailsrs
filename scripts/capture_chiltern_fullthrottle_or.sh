#!/usr/bin/env bash
# Experimento B — lanzar Open Rails (Explorer) para capturar baseline throttle 100 % / 120 s.
#
...
echo "=== Open Rails — Experimento B (Explorer, 100 % throttle) ==="
...
echo "  4. Throttle 100 %: D repetida hasta THROTTLEPERC ~100 en el HUD"
echo "     (Pullman 8 notches → notch 8 / máximo; NO te quedes en ~050)"
echo "  5. Despausa y dejá correr **120 s** de tiempo simulado"
...
if [[ -f "$SRC_CSV" ]]; then
  ...
  echo "  Si es una captura vieja (p. ej. throttle 50 %), renombrala antes:"
  echo "  mv \"$SRC_CSV\" \"${SRC_CSV}.bak.$(date +%Y%m%d%H%M%S)\""
  echo ""
fi
echo "Luego instalá el baseline:"
echo "  ./scripts/install_chiltern_fullthrottle_baseline.sh"
echo ""
